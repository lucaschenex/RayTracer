...
class Material {
    public:
        ...
        Material(Color ambient, Color diffuse, Color specular, float shiness, Color emmision = Color(0.0, 0.0, 0.0)){
            this->ka = ambient;
            this->kd = diffuse;
            this->ks = specular;
            this->sp = shiness;
...
Color get_color(Vector4f viewer, Vector4f normal, Vector4f intersect){
    ...
    if (pt_light_counter != 0) {
        for (int l = 0; l < pt_light_counter; l++) {
            ...
            Vector4f a(intersect[0]*1.01,intersect[1]*1.01,intersect[2]*1.01,1);
            testshadow = Ray(a, light);
            ...
            if(is_shadow==None){
                ...
                Color ambient1 = ka*pt_light_rgb;

                R = R + (diffuse1 + specular1+ambient1);
...
//            cout<<light<<endl;
             Vector4f b(intersect[0]*1.01,intersect[1]*1.01,intersect[2]*1.01,1);
             testshadow = Ray(b, light);
             is_shadow = Find_nearest(testshadow, objects);
...
Color trace(Ray ray,int TraceDepth){
    ...
    Ray reflection_ray = Ray(a, rflct);
    ...
    hit_index = -1;
    R = R + trace(reflection_ray,TraceDepth-1)*0.5;
    return R;
};
...
int main(int args, char* argv[]){
	...
	fov = 45.0f;
	camara_position = Vector4f(0.0f, 0.0f, 8.0f, 1.0f);
...
//	pt_xyz[0] = Vector4f(1,1,1,1.0);
	...
	// draw a sphere:
	float radius = 1.0f;
	Ambient_input = Color(0.0, 0.0, 0.0);
    Diffuse_input = Color(0.221, 0.22, 0.2);
    Specular_input = Color(0.214, 0.221, 0.223);
    Shiness_input = 30;
    ...
    Material mtrl = Material(Ambient_input, Diffuse_input, Specular_input, Shiness_input);


	Sphere test_sphere(test_translate, center, radius*2, mtrl);
	

    Vector4f center2 = Vector4f(2,2,2,1);
    Diffuse_input = Color(0.72, 0.7, 0.8);
    Specular_input = Color (0.44, 0.71, 0.6);
    Shiness_input = 10;
    mtrl = Material(Ambient_input, Diffuse_input, Specular_input, Shiness_input);
